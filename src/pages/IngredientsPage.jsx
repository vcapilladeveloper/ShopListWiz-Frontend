...
const IngredientsPage = () => {
    ...
    const handleOpenModal = (ingredient = null) => {
        console.log("Opening modal for ingredient: %o", ingredient);
        if (ingredient) {
            setEditingIngredient(ingredient);
            // Map API snake_case keys to form's camelCase keys
            const formDataFromIngredient = {
                // Start with initialFormState to ensure all fields are present
                ...initialFormState,
                // Override with common properties from ingredient
                english: ingredient.english || '',
                spanish: ingredient.spanish || '',
                catalan: ingredient.catalan || '',
                // Map API snake_case for nutritional values
                nutritionalValuesPer100G: ingredient.nutritional_values_per_100g
                    ...
                    : { ...initialFormState.nutritionalValuesPer100G }, // Fallback to default structure
                // Map API snake_case booleans to form's camelCase booleans
    ...
    const handleDelete = (ingredientId) => {
        if (window.confirm(t('ingredientsPage.confirmDelete'))) {
                        ...
                        {ingredients.map((ing) => (
                            <tr key={ing.id}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <button onClick={() => handleOpenModal(ing)} className="text-indigo-600 hover:text-indigo-900 mr-3">{t('common.edit')}</button>
                                    {/* <button onClick={() => handleDelete(ing.id)} className="text-red-600 hover:text-red-900">{t('common.delete')}</button> */}
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center p-4">
                    <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg max-h-full overflow-y-auto">
                        <h2 className="text-2xl font-semibold mb-6">{editingIngredient ? t('ingredientsPage.editIngredient') : t('ingredientsPage.addIngredient')}</h2>
                        <form onSubmit={handleSubmit}>
                            ...
                            <fieldset className="mb-4 border p-4 rounded-md">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2">
                                    ...
                                    <div>
                                        <label htmlFor="protein" className="block text-sm font-medium text-gray-700">{t('ingredientsPage.protein')}</label>
                                        <input type="number" name="protein" id="protein" value={formData.nutritionalValuesPer100G.protein_g} onChange={handleNutritionalChange} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" step="any" />
                                    </div>
                                    <div>
                                        <label htmlFor="carbohydrates" className="block text-sm font-medium text-gray-700">{t('ingredientsPage.carbohydrates')}</label>
                                        <input type="number" name="carbohydrates" id="carbohydrates" value={formData.nutritionalValuesPer100G.carbohydrates_g} onChange={handleNutritionalChange} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" step="any" />
                                    </div>
                                    <div>
                                        <label htmlFor="fat" className="block text-sm font-medium text-gray-700">{t('ingredientsPage.fat')}</label>
                                        <input type="number" name="fat" id="fat" value={formData.nutritionalValuesPer100G.fat_g} onChange={handleNutritionalChange} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" step="any" />
                                    </div>
                                    <div>
                                        <label htmlFor="fiber" className="block text-sm font-medium text-gray-700">{t('ingredientsPage.fiber')}</label>
                                        <input type="number" name="fiber" id="fiber" value={formData.nutritionalValuesPer100G.fiber_g} onChange={handleNutritionalChange} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" step="any" />
                                    </div>
                                    <div>
